function stapmex(hcpugpuaction,hcurrenttime,hcluttersimtimecpu,...
    ...
    hwaitbar_fill,clutter_validate,waitPanel,activeComp)
% This function stapcpu is only in support of STAPCPUGPUExample. It may be
...
maxRange = clutter_validate.MaximumRange;
patchAzWidth = clutter_validate.PatchAzimuthWidth;
compileClutter(maxRange, patchAzWidth,waitPanel,activeComp);
...
for m = 1:total_iter
    ...
    tcsim = tic;
    csig(:,:,storeidx) = stapclutter_mex(maxRange,patchAzWidth);
    csimtime = toc(tcsim);
...
function compileClutter(maxRangeArg, patchAzWidthArg,waitPanel,activeComp)

    persistent maxRange patchAzWidth;
    
    if ~exist('stapclutter_mex','file') || ...
            isempty(maxRange) || ...
            (maxRangeArg ~= maxRange) || ...
            (patchAzWidthArg ~= patchAzWidth)
        set(activeComp,'Enable','off');
        set(waitPanel,'Visible','on');
        drawnow;
        pause(.5);
        maxRange = maxRangeArg;
        patchAzWidth = patchAzWidthArg;
        codegen('stapclutter','-args',...
                {coder.Constant(maxRange),...
                 coder.Constant(patchAzWidth)});
        set(waitPanel,'Visible','off');
        set(activeComp,'Enable','on');
        drawnow;
    end
end
