...
[rx,mf,fs,noisepower,antennaarray] = ...
    getRectdata(fc,prop_speed,pd,pfa,max_range,min_rcs,range_res);
rectdata = struct('fs',fs,'propspeed',prop_speed,'fc',fc, ...
    'rxdata',rx,'mfcoeffs',mf,'noisepower',noisepower,'antennaarray',antennaarray);

[rx,xref,slope,fs,noisepower,antennaarray] = ...
    getFMCWdata(fc,prop_speed,pd,pfa,max_range,min_rcs,range_res);
fmcwdata = struct('fs',fs,'propspeed',prop_speed,'fc',fc, ...
    'rxdata',rx,'refsig',xref,'sweepslope',slope,'noisepower',noisepower,'antennaarray',antennaarray);

save('RangeAngleResponseExampleData.mat','rectdata','fmcwdata');
...
rngangresp = phased.RangeAngleResponse('SensorArray',antennaarray,'SampleRate',fs,...
    'PropagationSpeed',propspeed,'OperatingFrequency',fc);
...
xlabel('Angle (degrees)'); ylabel('Range (m)'); title('Range Angle Map');
...
plotResponse(rngangresp,rxdata(:,:,1),refsig, ...
    'Unit','db');

%% Helpers
function [rx_data,mfcoeffs,fs,noisepower,antennaarray] = getRectdata(fc,prop_speed,pd,pfa,max_range,min_rcs,range_res)
...
%% Targets
tgtpos = [[2000;500;0],[2000;-300;0],[3500;0;0]];
tgtvel = [[100;0;0],[0;0;0],[0;0;0]];
tgtrcs = [1.6 2.2 1.05];

%% Generate data
[rx_data,antennaarray] = simulateSystem(fc,waveform,transmitter,receiver,num_pulse_int, ...
...
function [rx_data,xref,sweepslope,fs,noisepower,antennaarray] = getFMCWdata(fc,prop_speed,pd,pfa,max_range,min_rcs,range_res)
...
%% Generate data
[rx_data,antennaarray] = simulateSystem(fc,waveform,transmitter,receiver,num_pulse_int,tgtpos,tgtvel,tgtrcs);
...
function [rxdata,antennaarray] = simulateSystem(fc,waveform,transmitter,receiver,num_pulse_int,tgtpos,tgtvel,tgtrcs)
...
num_ele = 8;
antennaarray = phased.ULA(num_ele,lambda/2,'Element',antenna);
...
rxdata = zeros(num_fast_time,num_ele,num_pulse_int);
