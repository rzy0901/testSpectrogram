function Pt = radareqpow(lambda,RNG,SNRdb,tau,varargin)
%radareqpow Radar equation to estimate peak transmit power
%   Pt = radareqpow(LAMBDA,RANGE,SNR,Tau) estimates the peak transmit power
%   (Watts) required for a radar to detect a target at the range, RANGE
%   (meters). The power estimate is based on the signal-to-noise-ratio, SNR
%   (dB), wavelength, LAMBDA (meters) and pulse width, Tau (seconds).
%
...
%
%   radareqpow(...,'RCS',RCS) specifies the target's radar cross section
%   RCS in square meters. If omitted RCS = 1 m^2. The function assumes a
%   non-fluctuating target (Swerling case 0).
%
%   radareqpow(...,'Ts',Ts) specifies the system noise temperature Ts
%   in kelvin (K). If omitted Ts = 290 K.
%
%   radareqpow(...,'Gain',G) specifies the antenna gain G in decibels (dB).
...
%
%   radareqpow(...,'Loss',L) specifies system losses L in decibels (dB). If
%   omitted L = 0 dB.
%
%   % Example:
%   %   Estimate the peak transmit power required for a radar operating at 
%   %   a frequency of 5.6 GHz to detect a target at 85.8 km. Assume an RCS
%   %   of 0.1 m^2 and a rectangular waveform with a pulse width of 0.2 
%   %   microseconds. The antenna gain 45 dB and the minimum detectable 
%   %   SNR is 20 dB. Assume no losses.
%
%   lambda = 3e8/5.6e9; RNG = 85800; SNR = 20; 
%   tau = 0.2e-6; RCS = 0.1; G = 45;  
%   Pt = radareqpow(lambda,RNG,SNR,tau,'rcs',RCS,'gain',G)
%
%   See also phased, radareqrng, radareqsnr.
...
[RCS, Ts, Gain, Loss] = parseInput(lambda,RNG,SNRdb,tau,varargin{:}); % Validate required inputs.
...
L   = db2pow(Loss);
SNR = db2pow(SNRdb);
Pt = ((4*pi)^3*k*Ts*Rtx^2*Rrx^2*L*SNR)/(tau*Gtx*RCS*Grx*lambda^2); % Watts

%-------------------------------------------
function [RCS, Ts, Gain, Loss] = parseInput(lambda,RNG,SNR,tau,varargin)

eml_assert_no_varsize(1:nargin,lambda,RNG,SNR,tau,varargin{:});
funName = 'radareqpow';

validateattributes(lambda,{'double'},{'positive','nonempty','scalar','finite'},...
    funName,'LAMBDA',1);

validateattributes(SNR,{'double'},{'real','nonempty','scalar'},...
    funName,'SNR',3);
