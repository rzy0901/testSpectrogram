classdef (Sealed,StrictDefaults) TwoRayChannel < phased.internal.AbstractLOSChannel
%TwoRayChannel Two-ray multipath propagation channel
%   H = phased.TwoRayChannel creates a two-ray channel System object, H.
%   This object simulates narrowband signal propagation through a two-ray
%   channel over a flat earth. A two-ray channel is a homogeneous,
%   isotropic medium like free space but with a reflecting boundary (often
%   the ground). A two-ray channel propagates a signal from a source to a
%   destination via two paths. The first path, the direct path, is the same
%   as in the free space. The second path, the ground reflected path,
%   propagates from the source to the boundary and then to the destination.
%   The object applies not only range-dependent time delay, gain, and phase
%   shift, but also weather related losses for both the direct path and the
%   ground reflection path to the input signal. The propagated signal can
%   either be specified as a single signal for both paths, or as two
%   separate signals along each path.
%
%   H = phased.TwoRayChannel(Name,Value) returns a two-ray propagation
...
%
%   % Example 1:
%   %   Calculate the result of propagating a signal in a two-ray 
...
%
%   % two-ray propagation
%   env_tworay = phased.TwoRayChannel('SampleRate',waveform.SampleRate);
...
%
%   env_tworay = phased.TwoRayChannel(...
...
%   phased.WidebandLOSChannel, phased.WidebandTwoRayChannel, fspl.
...
%#codegen
    ...
    methods
        function set.GroundReflectionCoefficient(obj,val)
            validateattributes(val,{'double'},{'row','finite'},...
                'TwoRayChannel','GroundReflectionCoefficient');
    ...
    methods
        function obj = TwoRayChannel(varargin)
            ...
            for pIdx = 1:numOfPropPaths
                if rem(pIdx,2)
                    ...
                    if obj.EnablePolarization
                        ...
                        % y component direction remains the same after
                        % reflection but z component direction is reversed
                        % after reflection.
                        ...
                        % in x, y, z
                        y(:,colidx) = ...
                            exp(-1i*2*pi*k*propdistance(pIdx)/lambda)/plossfactor(pIdx)*...
                            bsxfun(@times,x(:,colidx),...
                            exp(1i*2*pi*k*rspeed(pIdx)/lambda*(propdelay(pIdx)+(0:size(x,1)-1)'/Fs)));
    ...
    methods (Access = protected, Static, Hidden)
        function header = getHeaderImpl
          header = matlab.system.display.Header(...
              'Title',getString(message('phased:library:block:TwoRayChannelTitle')),...
              'Text',getString(message('phased:library:block:TwoRayChannelDesc')));
        end
        function groups = getPropertyGroupsImpl
            groups = matlab.system.display.Section(...
                'phased.TwoRayChannel');
            groups.PropertyList = groups.PropertyList([11:12 5:10 13:14 1:4 15:end]);
