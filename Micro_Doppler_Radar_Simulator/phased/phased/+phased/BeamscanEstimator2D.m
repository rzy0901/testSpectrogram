classdef (Sealed,StrictDefaults) BeamscanEstimator2D < phased.internal.AbstractSpectralDOA
%BeamscanEstimator2D 2-D Beamscan spatial spectrum estimator
%   H = phased.BeamscanEstimator2D creates a 2-D beamscan spatial spectrum
%   estimator System object, H. The object estimates the signal's spatial
%   spectrum using a narrowband conventional beamformer.
%
%   H = phased.BeamscanEstimator2D(Name,Value) creates a 2-D beamscan
%   spatial spectrum estimator object, H, with the specified property Name
...
%   equivalent.
%
%   BeamscanEstimator2D methods:
%
%   step         - Perform spatial spectrum estimation (see above)
%   release      - Allow property value and input characteristics changes
%   clone        - Create a 2-D beamscan spatial spectrum estimator object 
%                  with same property values
%   isLocked     - Locked status (logical)
%   <a href="matlab:help matlab.System/reset   ">reset</a>        - Reset states of 2D beamscan spatial spectrum estimator 
%                  object
%   plotSpectrum - Plot spatial spectrum
%
%   BeamscanEstimator2D properties:
%
...
%   beamscan = phased.BeamscanEstimator2D('SensorArray',ha,...
...
%   surf(beamscan.AzimuthScanAngles,beamscan.ElevationScanAngles,y);
...
%
%   See also phased, phased.BeamscanEstimator.
...
methods
    ...
    function obj = BeamscanEstimator2D(varargin)
...
methods (Access = protected)
    ...
    function titlestr = getSpectrumPlotTitle(obj)
        coder.extrinsic('phased.BeamscanEstimator2D.getSpectrumPlotTitleExtrinsic');
        titlestr = obj.getSpectrumPlotTitleExtrinsic();
    end
    function str = getIconImpl(obj) %#ok<MANU>
        str = sprintf('Beamscan\nSpectrum');
    end    
end
methods (Static, Hidden) 
    function titlestr = getSpectrumPlotTitleExtrinsic()
        titlestr = sprintf('%s %s',...
            getString(message('phased:doa:Beamscan2D')),...
...
methods (Static,Hidden,Access=protected)
    ...
    function header = getHeaderImpl
        header = matlab.system.display.Header(...
            'Title',getString(message('phased:library:block:BeamscanEstimator2DTitle')),...
            'Text',getString(message('phased:library:block:BeamscanEstimator2DDesc')));
    end    
end
end
