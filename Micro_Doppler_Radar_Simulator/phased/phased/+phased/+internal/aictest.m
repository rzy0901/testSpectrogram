function D = aictest(eigenvals,K,fb)
%This function is for internal use only. It may be removed in the future.

% aictest Estimate signal subspace dimension using AIC algorithm
% D = aictest(EIGENVALS,K,FB) returns an estimate of the signal subspace
% dimension using the AIC or AIC-FB algorithm. EIGENVALS are the
...
%#codegen
    ...
    if any(eigenvals==0)
       coder.internal.errorIf(any(eigenvals==0), ...
           'phased:phased:internal:aictest:ZeroEigvalues');
    end
    ...
    if fb
        for d=0:M-1,
            aux(d+1) = Ld(d,eigenvals,K,M) + d*(2*M-d+1)/2;   % Eq (7.510) in [1]
        end
    else
        for d=0:M-1,
            aux(d+1) = Ld(d,eigenvals,K,M) + d*(2*M-d);  % Eq (7.506) in [1]
        end
    end
