classdef (Hidden, Sealed, StrictDefaults) ComplexWGNSource < matlab.System
%This class is for internal use only. It may be removed in the future.

%ComplexWGNSource   Complex white Gaussian noise source
%   H = phased.internal.ComplexWGNSource creates a complex white Gaussian
%   noise source System object, H. The object generates complex white
%   Gaussian noise with specified dimensions.
%
%   H = phased.internal.ComplexWGNSource(Name,Value) creates a complex
...
%   specifying the dimensions of X.
%
...
%
%   hns = phased.internal.ComplexWGNSource;
    ...
    methods(Access = protected)
            ...
            if obj.SeedSource(1) == 'A' %Auto
                ...
                if cond
                    coder.internal.errorIf(cond, ...
                         'phased:ComplexWGNSource:CodegenNoSeed','SeedSource','Property');
                end
        ...
        function y = stepImpl(obj,pow,sz)
            ...
            if obj.pUseGlobalStream
                yr = randn(sz);
                yc = randn(sz);
                y = sqrt(pow/2)*(yr + 1i*yc);
            else
                rs = obj.cRandStream;
                y = sqrt(pow/2)*(randn(rs,sz)+1i*randn(rs,sz));
            end
        end
    end
end
