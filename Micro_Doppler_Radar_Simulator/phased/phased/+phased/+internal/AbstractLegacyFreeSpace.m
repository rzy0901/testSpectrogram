classdef (Hidden) AbstractLegacyFreeSpace < phased.internal.AbstractSampleRateEngine & ...
...
%#codegen
    ...
    methods (Access = protected)
        function obj = AbstractLegacyFreeSpace(varargin)
    ...
    methods (Access = protected)
        ...
        function setupImpl(obj,x,~,~,~,~)
            ...
            Fs = getSampleRate(obj,sz_x,1,obj.SampleRate);
            obj.pSampleRate = Fs;
            setRangeFactor(obj);
        ...
        function y = stepImpl(obj,x_inc,startPos,endPos,startVel,endVel)
            ...
            if obj.pIsInputStruct
                ...
                for chanidx = coder.unroll(1:numel(y))
                    for m = coder.unroll(1:num_fn)
                        y(chanidx).(fn(m)) = y_out(:,(chanidx-1)*num_fn+m);
                    end
                end
            else
                y = y_out;
